...
def reproducir_audio(mensaje):
    ...
    try:
        ...
        # Eliminar el archivo de audio despuÃ©s de la reproducciÃ³n para no acumular archivos
        os.remove('mensaje.mp3')
    except Exception as e:
        print(f"Error al reproducir el audio: {e}")

def report_access(status, breakPoint, person_id=None):
                ...
                if data:
                    for acceso in data:
                        if acceso['status'] == "Permitido":
                            ...
                            print(f"Acceso ID {id_acceso} actualizado como leÃ­do y abierto.")
                    
                    
                    # La lÃ­nea de cÃ³digo adicional
...
def capturar_foto():
    ...
    try:
        ...
        # Configurar cÃ¡mara para que sea mÃ¡s eficiente
        cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)
        cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)
        cap.set(cv2.CAP_PROP_FPS, 30)  # Establecer FPS para una captura mÃ¡s rÃ¡pida
        cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)  # Minimizar el buffer para respuesta mÃ¡s rÃ¡pida
...
def main(user_id):
    ...
    try:
        ...
        for filename in os.listdir(base_path):
            ...
            if os.path.isfile(image_path) and image_path.endswith(('.png', '.jpg', '.jpeg')):
                ...
                if encodings:
                    ...
                    print(f"No se encontrÃ³ rostro en la imagen de referencia: {image_path}")
        ...
        cap = cv2.VideoCapture(0)
        if not cap.isOpened():
            print("Error: no se puede acceder a la cÃ¡mara.")
            reproducir_audio("Error, no se puede acceder a la cÃ¡mara.")
            return False
        ...
        while True:
            ...
            if not face_locations:
                print("No se detectÃ³ ningÃºn rostro.")
            else:
    ...
    with mic as source:
        ...
        while True:
            ...
            try:
                ...
                print("No se entendiÃ³ el audio, intenta de nuevo.")
...
def consola_escuchar():
    """Escucha la entrada de consola y ejecuta la misma funciÃ³n que la palabra clave 'solicitud'"""
    while True:
        entrada = input()
        if entrada == '1':
            print("Se presiono '1' en la consola.")
            procesar_solicitud()
...
try:
    ...
    # Iniciar la revisiÃ³n de accesos permitidos en un hilo separado
    ...
    while True:
        ...
        if api_response:
            ...
            if "names" in api_response and "id" in api_response:
                ...
                # Llamar a la funciÃ³n main para verificar el reconocimiento facial
                if main(user_id):
